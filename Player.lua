...
function Player:init(map)
    ...
    self.x = map.mapWidth * map.tileWidth / 5
    self.y = map.mapHeight * map.tileHeight / 2

    self.dx = 0
    self.dy = 0

    self.texture = love.graphics.newImage('Graphics/blue.png')
...
function Player:render()
    ...
    if instructions == false then
        if self.score > 2 then
            love.graphics.draw(self.hearts, self.heart[7], 8 * map.tileWidth, map.tileHeight)
        else
            love.graphics.draw(self.hearts, self.heart[12], 8 * map.tileWidth, map.tileHeight)
        end

        if self.score > 1 then
            love.graphics.draw(self.hearts, self.heart[7], 6 * map.tileWidth, map.tileHeight)
        else
            love.graphics.draw(self.hearts, self.heart[12], 6 * map.tileWidth, map.tileHeight)
        end

        if self.score > 0 then
            love.graphics.draw(self.hearts, self.heart[7], 4 * map.tileWidth, map.tileHeight)
        else
            love.graphics.draw(self.hearts, self.heart[12], 4 * map.tileWidth, map.tileHeight)
        end
    end
end